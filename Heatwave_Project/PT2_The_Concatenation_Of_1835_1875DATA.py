...
#%% Number of times a times a certain
Columns = Swan_River_1830_1875.columns
Count = []
count = 0
Count.append(count)
for i in range(1,len(Columns)):
    Vec = Swan_River_1830_1875['{}'.format(Columns[i])]
    Vec = Vec.dropna()
    count = len(Vec)
    print(count)
    Count.append(count)
...
Cname.append('you')
for i in range(1,len(Columns)):
     Cname.append('{}'.format(Columns[i]))
    
    
Times_Temp_Recrded = [Cname, Count]